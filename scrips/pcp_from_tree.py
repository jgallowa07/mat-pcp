...
class PCPHelper:
    ...
    def pcp_list_to_df(self, pcps):
        # TODO, using the hard-coded bounderies (1-indexed) from the GTF, slice out the spike gene from the PCP's
...
def write_pcps(
    ...
    p1 = tree_path, coding_site_path, fasta_path, gtf_path
    p2 = (all_pcps_path)
    PCPHelper(*p1).make_and_write_pcps(*p2)
    return None
